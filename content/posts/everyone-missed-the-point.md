...
Stanford's ["Lost in the Middle"](https://arxiv.org/abs/2307.03172) paper showed that LLMs have a U-shaped attention curve — they process the beginning and end of their context well, but performance drops significantly for information in the middle. Amazon's research on ["Context Length Alone"](https://arxiv.org/abs/2510.05381) went further: even with *perfect retrieval*, reasoning performance degrades 13-85% as input length increases. Not because the model can't find the information — because sheer volume overwhelms its ability to think clearly. Adobe's [NoLiMa benchmark](https://arxiv.org/abs/2502.05167) quantified the damage: GPT-4o drops from 99.3% accuracy at 1K tokens to 69.7% at 32K tokens.

The loop isn't dumb. It's context management. Every iteration, the agent starts with a clean slate — fresh context, no accumulated noise, no "lost in the middle" degradation. JetBrains presented [research at NeurIPS 2025](https://blog.jetbrains.com/research/2025/12/efficient-context-management/) showing that simply masking old observations in agent systems boosted solve rates while being 52% cheaper than keeping full context.
